...
extension ChatControllerImpl {
    func openPollOptionContextMenu(optionId: Data, params: ChatControllerInteraction.LongTapParams) -> Void {
        ...
        let _ = combineLatest(
            ...
            let canVote = poll.pollId.namespace == Namespaces.Media.CloudPoll && !Namespaces.Message.allNonRegular.contains(message.id.namespace) && !Namespaces.Message.allEphemeral.contains(message.id.namespace) && !isPollEffectivelyClosed(message: message, poll: poll) && !isRestricted && (selectedOptions.isEmpty || !poll.revotingDisabled)
            if canVote {
