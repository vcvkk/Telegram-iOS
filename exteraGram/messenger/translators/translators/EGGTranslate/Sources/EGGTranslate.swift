import Foundationimport SwiftSignalKitimport utils
public enum TranslateFetchError {
    case network
}

private let egTranslateSessionConfiguration: URLSessionConfiguration = .ephemeral
private let egTranslateSession: URLSession = URLSession(configuration: egTranslateSessionConfiguration)
...
public func requestTranslateUrl(url: URL) -> Signal<String, TranslateFetchError> {
    return Signal { subscriber in
        ...
        let downloadTask: URLSessionDataTask = egTranslateSession.dataTask(with: request, completionHandler: { data, response, _ in
